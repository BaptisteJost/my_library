...
def so_V3_LA_noise(sensitivity_mode, f_sky, ell_max, delta_ell=1, beam_corrected=False):
    ...
    NTubes_UHF = 2.
    # sensitivity
    # converting these to per tube sensitivities
    S_LA_27 = np.array([61, 48, 35]) * np.sqrt(1./NTubes_LF)
    ...
    N_ell_T_LA = np.array([N_ell_T_27, N_ell_T_39, N_ell_T_93,
                           N_ell_T_145, N_ell_T_225, N_ell_T_280])
    ...
    # make an array of nosie curves for T
    N_ell_P_LA = np.array([N_ell_P_27, N_ell_P_39, N_ell_P_93,
...
def so_V3_SA_noise(sensitivity_mode, one_over_f_mode, SAC_yrs_LF, f_sky, ell_max, delta_ell=1,
    ...
    alpha_pol = np.array([-2.4, -2.4, -2.4, -2.4, -2.4, -2.4])
    ...
    MN_T_225 = W_T_225 * np.sqrt(A_arcmin)
    MN_T_280 = W_T_280 * np.sqrt(A_arcmin)
    Map_white_noise_levels = np.sqrt(
        2)*np.array([MN_T_27, MN_T_39, MN_T_93, MN_T_145, MN_T_225, MN_T_280])
    #print("white noise level: ",Map_white_noise_levels ,"[uK-arcmin]")
